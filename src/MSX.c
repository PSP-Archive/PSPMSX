...
loc_msx_save_settings(char *chFileName)
{
  ...
  if (FileDesc != (FILE *)0 ) {
    ...
    fprintf(FileDesc, "msx_delta_y=%d\n"        , MSX.msx_delta_y);
...
loc_msx_load_settings(char *chFileName)
{
  ...
  while (fgets(Buffer,512, FileDesc) != (char *)0) {
    ...
    *Scan = '\0';
    ...
    if (!strcasecmp(Buffer,"msx_delta_y"))    MSX.msx_delta_y = Value;
    else
...
# endif
  ...
  for (index = 0; index < MSX_MAX_SAVE_STATE; index++) {
    ...
    memset(&MSX.msx_save_state[index].date, 0, sizeof(ScePspDateTime));
...
# else
        if (TmpFile = sceIoOpen( TmpFileName, PSP_O_WRONLY|PSP_O_CREAT, 0777)) {
          sceIoWrite(TmpFile, zip_buffer, unzipped_size);
          sceIoClose(TmpFile);
          error = loc_load_rom(TmpFileName);
          remove(TmpFileName);
        }
...
# endif
  ...
  if (save_id < MSX_MAX_SAVE_STATE) {
    ...
    if (! error) {
      ...
      MSX.msx_save_state[save_id].thumb = 0;
      memset(&MSX.msx_save_state[save_id].date, 0, sizeof(ScePspDateTime));
...
msx_treat_command_key(int msx_idx)
{
  ...
  switch (msx_idx) 
  {
    ...
    break;
    case MSXK_C_DECDELTA:
      if (MSX.msx_delta_y > -37) MSX.msx_delta_y--;
    break;
    case MSXK_C_INCDELTA: 
      if (MSX.msx_delta_y <  37) MSX.msx_delta_y++;
    break;
